...
### Internal Imports ###
from experiments import direct_experiments as be
from training import direct_trainer as t
...
def initialize(training_params):
    ...
    best_objective_checkpoint = ModelCheckpoint(dirpath=checkpoints_path, filename='{epoch}_objective_loss', save_top_k=1, mode='min', monitor='Loss/Validation/objective_loss')
    best_mse_checkpoint = ModelCheckpoint(dirpath=checkpoints_path, filename='{epoch}_mse', save_top_k=1, mode='min', monitor='Loss/Validation/mse')
    ...
    best_ssim_checkpoint = ModelCheckpoint(dirpath=checkpoints_path, filename='{epoch}_ssim', save_top_k=1, mode='min', monitor='Loss/Validation/ssim')
    ...
    training_params['lightning_params']['callbacks'] = [checkpoint_callback, best_loss_checkpoint, best_objective_checkpoint, best_mse_checkpoint, best_mae_checkpoint, best_ssim_checkpoint, best_pcc_checkpoint, best_cd_checkpoint] 
...
def run():
    # EXAMPLE: run_training(be.runet_experiment_1())
    pass
